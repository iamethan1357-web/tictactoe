...
export default function FriendsList({ onInvite, showToast }: FriendsListProps) {
  ...
  const respondToRequest = async (friendshipId: string, action: string) => {
    try {
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="neo-card mb-4">
        <h3 className="font-semibold mb-3 text-sm" style={{ color: "#718096" }}>
        ...
        <form onSubmit={addFriend} className="flex gap-2">
          ...
          <button
            type="submit"
            disabled={adding}
            className="neo-btn-accent px-4 py-2 rounded-xl text-sm"
          >
            {adding ? "..." : "Add"}
          </button>
        </form>
            ...
            {pendingRequests.map((req) => (
              ...
              >
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => respondToRequest(req.friendshipId, "accept")}
                    className="neo-btn-accent text-xs px-3 py-1 rounded-lg"
                  >
                    Accept
                  </button>
                  <button
                    onClick={() => respondToRequest(req.friendshipId, "reject")}
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs" style={{ color: "#718096" }}>
                      @{friend.username} • W:{friend.wins} L:{friend.losses} D:{friend.draws}
                    </p>
                  </div>
                </div>
                <button
                  onClick={() => onInvite(friend.username)}
                  className="neo-btn-accent text-xs px-3 py-2 rounded-lg"
                >
                  ⚔️ Play
                </button>
              </div>
