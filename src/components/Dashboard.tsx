...
export default function Dashboard({ user, onLogout, onUserUpdate }: { user: UserData; onLogout: () => void; onUserUpdate: (u: UserData) => void }) {
  ...
  const startAi = async (diff: string, level?: number, isDaily?: boolean) => {
    try { const r = await fetch("/api/game/ai", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "create", difficulty: diff, level: level || null, isDaily: isDaily || false }) }); const d = await r.json(); if (d.game) { setGameId(d.game.id); setSelLevel(level || null); setDaily(isDaily || false); setScreen("ai"); } } catch { flash("Failed"); }
  };
  ...
  if (screen === "ai" && gameId) return <Page onBack={back}><GameBoard gameId={gameId} mode="ai" user={user} level={selLevel} isDaily={daily} onGameEnd={refreshUser} /></Page>;
    ...
    <div className="min-h-screen p-4">
      ...
      <div style={{ maxWidth: 460, margin: "0 auto", paddingLeft: 40 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {/* 100 Levels */}
          <button onClick={() => setScreen("levels")} className="neo anim-slide" style={{ textAlign: "left", cursor: "pointer", width: "100%", border: "none" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ flex: 1 }}>
                <h3 style={{ ...tf, fontSize: 22, color: "var(--blue)" }}>📝 100 Levels</h3>
                <p style={{ fontSize: 14, color: "var(--ink-mid)", marginTop: 3 }}>each page harder than the last</p>
                <div style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 10 }}>
                  <div className="progress-neo" style={{ flex: 1 }}>
                    <div className="progress-fill" style={{ width: `${Math.min(user.currentLevel, 100)}%` }} />
                  </div>
                  <span style={{ ...tf, fontSize: 15, color: "var(--blue)" }}>{user.currentLevel}/100</span>
                </div>
              </div>
              <span style={{ fontSize: 24, color: "var(--ink-light)", marginLeft: 14 }}>→</span>
            </div>
          </button>

          {/* Daily */}
          <button onClick={() => startAi("hard", undefined, true)} className="neo anim-slide" style={{ textAlign: "left", cursor: "pointer", width: "100%", border: "none" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                <h3 style={{ ...tf, fontSize: 22, color: "var(--orange)" }}>📅 Daily Challenge</h3>
                <p style={{ fontSize: 14, color: "var(--ink-mid)", marginTop: 3 }}>new puzzle scribbled every day</p>
              </div>
              <span style={{ fontSize: 24, color: "var(--ink-light)" }}>→</span>
            </div>
          </button>

          {/* Friends */}
          <button onClick={() => setScreen("friends")} className="neo anim-slide" style={{ textAlign: "left", cursor: "pointer", width: "100%", border: "none" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                <h3 style={{ ...tf, fontSize: 22, color: "var(--green)" }}>👥 Play with Friends</h3>
                <p style={{ fontSize: 14, color: "var(--ink-mid)", marginTop: 3 }}>invite by username</p>
              </div>
              <span style={{ fontSize: 24, color: "var(--ink-light)" }}>→</span>
            </div>
          </button>
        ...
        <div style={{ textAlign: "center", margin: "28px 0 14px", fontSize: 12, color: "var(--ink-light)" }}>— ✏️ scribbled on graph paper —</div>
      </div>
    </div>
  );
}

function Page({ onBack, children }: { onBack: () => void; children: React.ReactNode }) {
  return (
    <div className="min-h-screen p-4">
      <div style={{ maxWidth: 460, margin: "0 auto", paddingLeft: 40 }}>
        <button onClick={onBack} className="neo-btn" style={{ marginBottom: 16, fontSize: 15, fontFamily: "inherit" }}>← back</button>
        {children}
      </div>
    </div>
  );
