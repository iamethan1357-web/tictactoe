...
export default function LevelsGrid({ currentLevel, onSelectLevel }: LevelsGridProps) {
  ...
  const getDifficultyColor = (level: number) => {
    ...
    if (level <= 40) return "#00cec9";
    if (level <= 60) return "#fdcb6e";
    if (level <= 80) return "#e17055";
    return "#d63031";
  };
  ...
  // Group levels by sections of 10
  const sections = Array.from({ length: 10 }, (_, i) => ({
    start: i * 10 + 1,
    end: (i + 1) * 10,
    label: getDifficultyLabel(i * 10 + 5),
    emoji: getDifficultyEmoji(i * 10 + 5),
    color: getDifficultyColor(i * 10 + 5),
  }));
  ...
  return (
    <div className="animate-fade-in">
      ...
      <p className="text-sm mb-4" style={{ color: "#718096" }}>
        Complete each level to unlock the next. Difficulty increases as you progress!
      </p>
      ...
      <div className="neo-card mb-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-sm font-bold" style={{ color: "#6c5ce7" }}>
            {Math.min(currentLevel - 1, 100)}/100
          </span>
        </div>
        <div className="w-full h-3 rounded-full" style={{ background: "#d1d9e6" }}>
          <div
            className="h-3 rounded-full transition-all"
            style={{
              width: `${Math.min(currentLevel - 1, 100)}%`,
      ...
      <div className="space-y-4">
        {sections.map((section) => (
          <div key={section.start} className="neo-card">
            ...
            >
              {section.emoji} {section.label} (Levels {section.start}-{section.end})
            </h3>
            <div className="grid grid-cols-5 gap-2">
              {Array.from({ length: 10 }, (_, i) => {
                const lvl = section.start + i;
                ...
                return (
                  <button
                    ...
                    className={`level-badge ${
                      completed ? "completed" : isCurrent ? "current" : "locked"
                    }`}
                    title={
                      completed
                        ? `Level ${lvl} ✅`
